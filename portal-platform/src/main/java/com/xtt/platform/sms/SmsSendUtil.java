...
public class SmsSendUtil {
	...
	static {
		...
		try {
			Properties properties = new ConfigFileUtil()
					.readProperty("/sms.properties");
			smsUrl = properties.getProperty("smsUrl");
            uid = properties.getProperty("uid");
            secretKey = properties.getProperty("secretKey");
            smsTemplate = new String(properties.getProperty("smsTemplate")
	...
	public static String sendSmsPost(String mobiles, String code) {
		...
		try {
			...
			String smsParam = "uid=" + uid + "&secret=" + secret + "&mobiles="
					+ mobiles + "&content=" + URLEncoder.encode(content,"UTF-8")
                    + "&type=sms";
			res = HttpSend.postSend(smsUrl, smsParam);
			LOG.info("/sendSmsPost----->mobiles:" + mobiles + ",content:" + content
					+ "----->" + res);
		} catch (Exception e) {
			LOG.error(e, e);
		}
	...
	public static String sendSmsGet(String mobiles, String code) {
		...
		try {
			...
			LOG.info("/sendSmsGet----->mobiles:" + mobiles + ",content:" + content
					+ "----->" + res);
		} catch (Exception e) {
			LOG.error(e, e);
		}
